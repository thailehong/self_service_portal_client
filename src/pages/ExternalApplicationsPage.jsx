...
export function ExternalApplicationsPage({ groupId }) {
  const [favoriteIds, setFavoriteIds] = useState([]);
  const [favoriteError, setFavoriteError] = useState("");
  const [savingFavoriteIds, setSavingFavoriteIds] = useState([]);
  const group = externalApplicationGroups.find((item) => item.id === groupId);
  const applications = applicationRegistry.filter(
